...
export default class HardwareAdminDashboard extends LightningElement {
  ...
  @track editOs = "";
  @track editModel = "";
  @track editProcessorGeneration = "";
  @track editUnifiedMemory = "";
  @track editStorageCapacity = "";
  @track editMacOsVersion = "";
  @track editPowerAdapterProvided = false;
  ...
  get isLaptopOrCpu() {
    return (
      this.newHardware.type === "Laptop" || this.newHardware.type === "CPU"
    );
  }

  get isEditLaptopOrCpu() {
    return this.editType === "Laptop" || this.editType === "CPU";
  }

  get isSelectedLaptopOrCpu() {
    return (
      this.selectedHardware &&
      (this.selectedHardware.Type__c === "Laptop" ||
        this.selectedHardware.Type__c === "CPU")
    );
  }

  get imageLabel() {
    return this.isLaptopOrCpu ? "Device Picture" : "Hardware Image";
  }

  get editImageLabel() {
    return this.isEditLaptopOrCpu ? "Device Picture" : "Hardware Image";
  }

  get isNewNotesRequired() {
    return (
      this.newHardware.status === "Under Maintenance" ||
      this.newHardware.status === "Retired"
    );
  }

  get isEditNotesRequired() {
    return (
      this.editStatus === "Under Maintenance" || this.editStatus === "Retired"
    );
  }
        ...
        statusClass:
          item.Status__c === "Available"
            ...
            : item.Status__c === "Assigned"
              ...
              : item.Status__c === "Under Maintenance"
                ? "slds-badge slds-theme_warning"
                : "slds-badge slds-theme_error",
        showCautionBtn:
          item.Assigned_To__c && item.Life_Span_Consumed_Percentage__c >= 100
      }));
    } catch (error) {
  ...
  async loadRequests() {
    ...
    try {
      ...
      this.requestList = data.map((req) => ({
        ...
        statusClass:
          req.Status__c === "Pending TSA"
            ? "slds-badge slds-theme_warning"
            : req.Status__c === "Forwarded to HR/Manager"
              ? "slds-badge slds-badge_lightest"
              : req.Status__c === "Fulfilled" || req.Status__c === "Approved"
                ? "slds-badge slds-theme_success"
                : "slds-badge slds-theme_error"
      }));
    } catch (error) {
  ...
  get pendingTickets() {
    return this.filteredTickets
      ? this.filteredTickets.filter(
          (t) =>
            t.Status__c === "Created Ticket" ||
            t.Status__c === "Forwarded to Admin"
        )
      : [];
  }
  ...
  get solvedTickets() {
    return this.filteredTickets
      ? this.filteredTickets.filter(
          (t) =>
            t.Status__c === "Solved by Admin" ||
            t.Status__c === "Solved by HR/Manager" ||
            t.Status__c === "Hardware will be Delivered"
        )
      : [];
  }
  ...
  get filteredInventoryList() {
    ...
    return this.inventoryList.filter((item) => {
      ...
      const empName = item.Assigned_To__r
        ? item.Assigned_To__r.Name.toLowerCase()
        : "";
  ...
  handleHardwareChange(event) {
    ...
    if (event.target.type === "checkbox") {
      this.newHardware[field] = event.target.checked;
    } else {
  ...
  async submitNewHardware() {
    ...
    if (
      !this.newHardware.name ||
      !this.newHardware.type ||
      !this.newHardware.status ||
      !this.newHardware.location
    ) {
      this.showToast("Error", "Please fill all required fields.", "error");
      return;
    }

    let yom = this.newHardware.yearOfManufacture
      ? parseInt(this.newHardware.yearOfManufacture, 10)
      : null;
    let els = this.newHardware.expectedLifeSpan
      ? parseInt(this.newHardware.expectedLifeSpan, 10)
      : null;
    ...
    try {
      await addHardware({
        ...
        model: this.newHardware.model,
        processorGeneration: this.newHardware.processorGeneration,
        unifiedMemory: this.newHardware.unifiedMemory,
        storageCapacity: this.newHardware.storageCapacity,
        macOsVersion: this.newHardware.macOsVersion,
        powerAdapterProvided: this.newHardware.powerAdapterProvided,
  ...
  openEditModal(event) {
    ...
    if (hw) {
      ...
      this.editModel = hw.Model__c || "";
      this.editProcessorGeneration = hw.Processor_Generation__c || "";
      this.editUnifiedMemory = hw.Unified_Memory_RAM__c || "";
      this.editStorageCapacity = hw.Storage_Capacity_SSD__c || "";
      this.editMacOsVersion = hw.OS_Version__c || "";
      this.editPowerAdapterProvided = !!hw.Power_Adapter_Provided__c;
  ...
  closeEditModal() {
    ...
    this.editOs = "";
    this.editModel = "";
    this.editProcessorGeneration = "";
    this.editUnifiedMemory = "";
    this.editStorageCapacity = "";
    this.editMacOsVersion = "";
    this.editPowerAdapterProvided = false;
    this.editShowCropper = false;
    this.editCroppedImageBase64 = "";
  ...
  handleEditStatusChange(event) {
    ...
    if (
      this.editStatus === "Under Maintenance" ||
      this.editStatus === "Retired"
    ) {
      this.editAssignedToId = null;
    }
  }
  ...
  handleEditModelChange(event) {
    this.editModel = event.target.value;
  }

  handleEditProcessorChange(event) {
    this.editProcessorGeneration = event.target.value;
  }

  handleEditMemoryChange(event) {
    this.editUnifiedMemory = event.target.value;
  }

  handleEditStorageChange(event) {
    this.editStorageCapacity = event.target.value;
  }

  handleEditMacOsChange(event) {
    this.editMacOsVersion = event.target.value;
  }

  handleEditPowerAdapterChange(event) {
    this.editPowerAdapterProvided = event.target.checked;
  }
  ...
  async saveEditHardware() {
    ...
    let yom = this.editYearOfManufacture
      ? parseInt(this.editYearOfManufacture, 10)
      : null;
    let els = this.editExpectedLifeSpan
      ? parseInt(this.editExpectedLifeSpan, 10)
      : null;
    ...
    try {
      await updateHardware({
        ...
        processorGeneration: this.editProcessorGeneration,
        unifiedMemory: this.editUnifiedMemory,
        storageCapacity: this.editStorageCapacity,
        macOsVersion: this.editMacOsVersion,
        powerAdapterProvided: this.editPowerAdapterProvided,
        callerId: this.callerId
      });
      this.closeEditModal();
      await LightningAlert.open({
  ...
  async submitRequestAction() {
    ...
    if (this.requestActionType === "Fulfill") {
      newStatus = "Fulfilled";
      successMsg =
        "Request marked as fulfilled. Assign hardware in the inventory tab!";
  ...
  async loadTickets() {
    ...
    try {
      ...
      this.ticketList = data.map((ticket) => ({
        ...
        isOpen:
          ticket.Status__c === "Created Ticket" ||
          ticket.Status__c === "Forwarded to Admin",
        statusClass:
          ticket.Status__c === "Solved by Admin" ||
          ticket.Status__c === "Solved by HR/Manager"
            ? "slds-badge slds-theme_success"
            : ticket.Status__c === "Created Ticket" ||
                ticket.Status__c === "Forwarded to Admin"
              ? "slds-badge slds-theme_warning"
  ...
  filterTickets() {
    ...
    this.filteredTickets = this.ticketList.filter(
      (ticket) =>
        ticket.Employee__r?.Name?.toLowerCase().includes(
          this.ticketSearchKey
        ) ||
        ticket.Hardware__r?.Name?.toLowerCase().includes(
          this.ticketSearchKey
        ) ||
  ...
  async submitTicketAction() {
    ...
    if (
      this.ticketActionType === "Resolve" &&
      this.createNewRequest &&
      !this.newRequestHwType
    ) {
      this.showToast(
        "Error",
        "Please select a hardware type for the new request.",
        "error"
      );
      return;
    }
